...
MODULE MOD_Vars_1DAccFluxes
   ...
   real(r8), allocatable :: a_ch4_ebul_tot          (:)
   real(r8), allocatable :: a_ch4_prod_tot          (:)
   real(r8), allocatable :: a_ch4_oxid_tot          (:)
   ...
   real(r8), allocatable :: a_ch4_aere_depth_unsat        (:,:)
   real(r8), allocatable :: a_ch4_aere_depth_sat          (:,:)
   real(r8), allocatable :: a_ch4_tran_depth_unsat        (:,:)
   real(r8), allocatable :: a_ch4_tran_depth_sat          (:,:)
   real(r8), allocatable :: a_o2_aere_depth_unsat         (:,:)
   real(r8), allocatable :: a_o2_aere_depth_sat           (:,:)
   real(r8), allocatable :: a_ch4_ebul_depth_unsat        (:,:)
   real(r8), allocatable :: a_ch4_ebul_depth_sat          (:,:)
   real(r8), allocatable :: a_o2stress_unsat              (:,:)
   real(r8), allocatable :: a_o2stress_sat                (:,:)
   real(r8), allocatable :: a_ch4stress_unsat             (:,:)
   real(r8), allocatable :: a_ch4stress_sat               (:,:)
   real(r8), allocatable :: a_ch4_surf_flux_tot_unsat     (:)
   real(r8), allocatable :: a_ch4_surf_flux_tot_sat       (:)
   real(r8), allocatable :: a_ch4_surf_aere_unsat         (:)
   real(r8), allocatable :: a_ch4_surf_aere_sat           (:)
   real(r8), allocatable :: a_ch4_surf_ebul_unsat         (:)
   real(r8), allocatable :: a_ch4_surf_ebul_sat           (:)
   real(r8), allocatable :: a_ch4_surf_diff_unsat         (:)
   real(r8), allocatable :: a_ch4_surf_diff_sat           (:)
   real(r8), allocatable :: a_ch4_ebul_tot_unsat          (:)
   real(r8), allocatable :: a_ch4_ebul_tot_sat            (:)
   real(r8), allocatable :: a_ch4_prod_tot_unsat          (:)
   real(r8), allocatable :: a_ch4_prod_tot_sat            (:)
   real(r8), allocatable :: a_ch4_oxid_tot_unsat          (:)
   real(r8), allocatable :: a_ch4_oxid_tot_sat            (:)

   real(r8), allocatable :: a_totcolch4_unsat             (:)
   ...
   IMPLICIT NONE
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            ...
            allocate (a_ch4_ebul_tot                (numpatch))
            allocate (a_ch4_prod_tot                (numpatch))
            allocate (a_ch4_oxid_tot                (numpatch))
            ...
            allocate (a_ch4_aere_depth_unsat  (nl_soil,numpatch))
            allocate (a_ch4_aere_depth_sat    (nl_soil,numpatch))
            allocate (a_ch4_tran_depth_unsat  (nl_soil,numpatch))
            allocate (a_ch4_tran_depth_sat    (nl_soil,numpatch))
            allocate (a_o2_aere_depth_unsat   (nl_soil,numpatch))
            allocate (a_o2_aere_depth_sat     (nl_soil,numpatch))
            allocate (a_ch4_ebul_depth_unsat  (nl_soil,numpatch))
            allocate (a_ch4_ebul_depth_sat    (nl_soil,numpatch))
            allocate (a_o2stress_unsat        (nl_soil,numpatch))
            allocate (a_o2stress_sat          (nl_soil,numpatch))
            allocate (a_ch4stress_unsat       (nl_soil,numpatch))
            allocate (a_ch4stress_sat         (nl_soil,numpatch))
            allocate (a_ch4_surf_flux_tot_unsat     (numpatch))
            allocate (a_ch4_surf_flux_tot_sat       (numpatch))
            allocate (a_ch4_surf_aere_unsat         (numpatch))
            allocate (a_ch4_surf_aere_sat           (numpatch))
            allocate (a_ch4_surf_ebul_unsat         (numpatch))
            allocate (a_ch4_surf_ebul_sat           (numpatch))
            allocate (a_ch4_surf_diff_unsat         (numpatch))
            allocate (a_ch4_surf_diff_sat           (numpatch))
            allocate (a_ch4_ebul_tot_unsat          (numpatch))
            allocate (a_ch4_ebul_tot_sat            (numpatch))
            allocate (a_ch4_prod_tot_unsat          (numpatch))
            allocate (a_ch4_prod_tot_sat            (numpatch))
            allocate (a_ch4_oxid_tot_unsat          (numpatch))
            allocate (a_ch4_oxid_tot_sat            (numpatch))

            allocate (a_totcolch4_unsat             (numpatch))
   ...
   IMPLICIT NONE
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            ...
            deallocate (a_ch4_ebul_tot           )
            deallocate (a_ch4_prod_tot           )
            deallocate (a_ch4_oxid_tot           )
            ...
            !!!!                                         sum data (unsaturated / saturated)
            !!!! --------------------------------------------------------------------------------------------------------
            deallocate (a_net_methane_unsat      )
            ...
            deallocate (a_o2_oxid_depth_sat      )
            deallocate (a_ch4_aere_depth_unsat   )
            deallocate (a_ch4_aere_depth_sat     )
            deallocate (a_ch4_tran_depth_unsat   )
            deallocate (a_ch4_tran_depth_sat     )
            deallocate (a_o2_aere_depth_unsat    )
            deallocate (a_o2_aere_depth_sat      )
            deallocate (a_ch4_ebul_depth_unsat   )
            deallocate (a_ch4_ebul_depth_sat     )
            deallocate (a_o2stress_unsat         )
            deallocate (a_o2stress_sat           )
            deallocate (a_ch4stress_unsat        )
            deallocate (a_ch4stress_sat          )
            deallocate (a_ch4_surf_flux_tot_unsat)
            deallocate (a_ch4_surf_flux_tot_sat  )
            deallocate (a_ch4_surf_aere_unsat    )
            deallocate (a_ch4_surf_aere_sat      )
            deallocate (a_ch4_surf_ebul_unsat    )
            deallocate (a_ch4_surf_ebul_sat      )
            deallocate (a_ch4_surf_diff_unsat    )
            deallocate (a_ch4_surf_diff_sat      )
            deallocate (a_ch4_ebul_tot_unsat     )
            deallocate (a_ch4_ebul_tot_sat       )
            deallocate (a_ch4_prod_tot_unsat     )
            deallocate (a_ch4_prod_tot_sat       )
            deallocate (a_ch4_oxid_tot_unsat     )
            deallocate (a_ch4_oxid_tot_sat       )

            deallocate (a_totcolch4_unsat        )
   ...
   SUBROUTINE FLUSH_acc_fluxes ()
      ...
      IF (p_is_worker) THEN
         ...
         IF (numpatch > 0) THEN
            ...
            a_ch4_ebul_tot           (:)  = spval
            a_ch4_prod_tot           (:)  = spval
            a_ch4_oxid_tot           (:)  = spval
            ...
            a_o2_oxid_depth_sat    (:,:)  = spval
            a_ch4_aere_depth_unsat (:,:)  = spval
            a_ch4_aere_depth_sat   (:,:)  = spval
            a_ch4_tran_depth_unsat (:,:)  = spval
            a_ch4_tran_depth_sat   (:,:)  = spval
            a_o2_aere_depth_unsat  (:,:)  = spval
            a_o2_aere_depth_sat    (:,:)  = spval
            a_ch4_ebul_depth_unsat (:,:)  = spval
            a_ch4_ebul_depth_sat   (:,:)  = spval
            a_o2stress_unsat       (:,:)  = spval
            a_o2stress_sat         (:,:)  = spval
            a_ch4stress_unsat      (:,:)  = spval
            a_ch4stress_sat        (:,:)  = spval
            a_ch4_surf_flux_tot_unsat(:)  = spval
            a_ch4_surf_flux_tot_sat  (:)  = spval
            a_ch4_surf_aere_unsat    (:)  = spval
            a_ch4_surf_aere_sat      (:)  = spval
            a_ch4_surf_ebul_unsat    (:)  = spval
            a_ch4_surf_ebul_sat      (:)  = spval
            a_ch4_surf_diff_unsat    (:)  = spval
            a_ch4_surf_diff_sat      (:)  = spval
            a_ch4_ebul_tot_unsat     (:)  = spval
            a_ch4_ebul_tot_sat       (:)  = spval
            a_ch4_prod_tot_unsat     (:)  = spval
            a_ch4_prod_tot_sat       (:)  = spval
            a_ch4_oxid_tot_unsat     (:)  = spval
            a_ch4_oxid_tot_sat       (:)  = spval

            a_totcolch4_unsat        (:)  = spval
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
           ...
           !CALL acc1d (sum_irrig_count    ,   a_sum_irrig_count    )
            ...
            CALL acc1d (ch4_ebul_tot                        ,   a_ch4_ebul_tot                        )
            CALL acc1d (ch4_prod_tot                        ,   a_ch4_prod_tot                        )
            CALL acc1d (ch4_oxid_tot                        ,   a_ch4_oxid_tot                        )
            ...
            CALL acc2d (ch4_aere_depth_unsat                ,   a_ch4_aere_depth_unsat                )
            CALL acc2d (ch4_aere_depth_sat                  ,   a_ch4_aere_depth_sat                  )
            CALL acc2d (ch4_tran_depth_unsat                ,   a_ch4_tran_depth_unsat                )
            CALL acc2d (ch4_tran_depth_sat                  ,   a_ch4_tran_depth_sat                  )
            CALL acc2d (o2_aere_depth_unsat                 ,   a_o2_aere_depth_unsat                 )
            CALL acc2d (o2_aere_depth_sat                   ,   a_o2_aere_depth_sat                   )
            CALL acc2d (ch4_ebul_depth_unsat                ,   a_ch4_ebul_depth_unsat                )
            CALL acc2d (ch4_ebul_depth_sat                  ,   a_ch4_ebul_depth_sat                  )
            CALL acc2d (o2stress_unsat                      ,   a_o2stress_unsat                      )
            CALL acc2d (o2stress_sat                        ,   a_o2stress_sat                        )
            CALL acc2d (ch4stress_unsat                     ,   a_ch4stress_unsat                     )
            CALL acc2d (ch4stress_sat                       ,   a_ch4stress_sat                       )
            CALL acc1d (ch4_surf_flux_tot_unsat             ,   a_ch4_surf_flux_tot_unsat             )
            CALL acc1d (ch4_surf_flux_tot_sat               ,   a_ch4_surf_flux_tot_sat               )
            CALL acc1d (ch4_surf_aere_unsat                 ,   a_ch4_surf_aere_unsat                 )
            CALL acc1d (ch4_surf_aere_sat                   ,   a_ch4_surf_aere_sat                   )
            CALL acc1d (ch4_surf_ebul_unsat                 ,   a_ch4_surf_ebul_unsat                 )
            CALL acc1d (ch4_surf_ebul_sat                   ,   a_ch4_surf_ebul_sat                   )
            CALL acc1d (ch4_surf_diff_unsat                 ,   a_ch4_surf_diff_unsat                 )
            CALL acc1d (ch4_surf_diff_sat                   ,   a_ch4_surf_diff_sat                   )
            CALL acc1d (ch4_ebul_tot_unsat                  ,   a_ch4_ebul_tot_unsat                  )
            CALL acc1d (ch4_ebul_tot_sat                    ,   a_ch4_ebul_tot_sat                    )
            CALL acc1d (ch4_prod_tot_unsat                  ,   a_ch4_prod_tot_unsat                  )
            CALL acc1d (ch4_prod_tot_sat                    ,   a_ch4_prod_tot_sat                    )
            CALL acc1d (ch4_oxid_tot_unsat                  ,   a_ch4_oxid_tot_unsat                  )
            CALL acc1d (ch4_oxid_tot_sat                    ,   a_ch4_oxid_tot_sat                    )
            CALL acc1d (totcolch4_unsat                     ,   a_totcolch4_unsat                     )
