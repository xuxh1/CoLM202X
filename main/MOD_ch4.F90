...
contains
	...
	subroutine ch4 (istep,idate,patchtype,&!input
		...
		call print_var(finundated, 'finundated', idate)
		...
		do sat= 0, 1
			if (sat==0) then ! unsaturated
				...
				call print_var(zwt_unsat, 'ch4 unsat zwt_unsat', idate)
				call print_var(wliq_soisno_unsat, 'ch4 unsat wliq_soisno_unsat', idate)
				call print_var(wice_soisno_unsat, 'ch4 unsat wice_soisno_unsat', idate)
				call print_var(wdsrf_unsat, 'ch4 unsat wdsrf_unsat', idate)
				call print_var(jwt_unsat, 'ch4 unsat jwt_unsat', idate)

				do j=1,nl_soil
					if (DEF_CH4%use_vertical_redoxlag .and. j > jwt_unsat .and. redoxlags_vertical > 0._r8) then ! saturated currently
			...
			elseif (sat==1) then ! saturated
				...
				call print_var(wice_soisno_sat, 'ch4 sat wice_soisno_sat', idate)
				call print_var(wdsrf_sat, 'ch4 sat wdsrf_sat', idate)
				call print_var(jwt_sat, 'ch4 sat jwt_sat', idate)
				...
				! Calculate CH4 production in each soil layer
				call ch4_prod ( idate, patchtype, sat, jwt_sat, finundated, finundated_lag, rr, deltim, &
					z_soisno, dz_soisno, zi_soisno, t_soisno, &
		...
		if (istep /= 1) then
			...
			err1 = errch4_sat*finundated+errch4_unsat*(1.0_r8 - finundated)
	...
	!-----------------------------------------------------------------------
	subroutine ch4_prod (idate,patchtype,sat,jwt,finundated,finundated_lag,rr,deltim,& !input
		z_soisno,dz_soisno,zi_soisno,t_soisno,&
		...
		do j=1,nl_soil
			...
			call print_var(f_ch4_adj,'ch4_prod 1 f_ch4_adj',idate,j,.true.)

			! Adjust DEF_CH4%f_ch4 by the ratio
			f_ch4_adj = DEF_CH4%f_ch4 * t_fact_ch4

			call print_var(f_ch4_adj,'ch4_prod 2 f_ch4_adj',idate,j,.true.)
			...
			call print_var(f_ch4_adj,'ch4_prod 3 f_ch4_adj',idate,j,.true.)
			...
			call print_var(f_ch4_adj,'ch4_prod 4 f_ch4_adj',idate,j,.true.)
			...
			call print_var(f_ch4_adj,'ch4_prod 5 f_ch4_adj',idate,j,.true.)
			...
			call print_var(f_ch4_adj,'ch4_prod f_ch4_adj',idate,j,.true.)
			call print_var(base_decomp,'ch4_prod base_decomp',idate,j,.true.)
			call print_var(partition_z,'ch4_prod partition_z',idate,j,.true.)
			call print_var(ch4_prod_depth(j),'ch4_prod ch4_prod_depth(j)',idate,j,.true.)
	...
	subroutine ch4_tran (idate,patchtype, &
		...
		do j = 1,nl_soil
			...
			call print_var(conc_ch4(j),'ch4_tran 1 conc_ch4(j)',idate,j,.true.)
			...
			if (j == 0) then
				...
				if (j <= jwt) then  ! Above the WT
					...
					do s =1,2
						...
						call print_var(epsilon_t(j,s),'ch4_tran epsilon_t(j,s)',idate,j,.true.)
					end do
					conc_ch4_rel(j) = conc_ch4(j)/epsilon_t(j,1)
					conc_o2_rel(j)  = conc_o2(j) /epsilon_t(j,2)
					call print_var(conc_ch4_aqu_porsl(j),'ch4_tran conc_ch4_aqu_porsl(j)',idate,j,.true.)
					call print_var(conc_o2_aqu_porsl(j),'ch4_tran conc_o2_aqu_porsl(j)',idate,j,.true.)
				end if
			end if
			call print_var(conc_ch4_rel(j),'ch4_tran 1 conc_ch4_rel(j)',idate,j,.true.)
			call print_var(conc_o2_rel(j),'ch4_tran 1 conc_o2_rel(j)',idate,j,.true.)
		end do
		...
		do s = 1, 2 ! 1=CH4; 2=O2; 3=CO2
			...
			if (s == 1) then  ! CH4
				...
				do j = 1,nl_soil
					...
					epsilon_t_old(j,s) = epsilon_t(j,s)
					source_old(j,s) = source(j,s)
					call print_var(at(j) ,'ch4_tran at(j)',idate,j,.false.,s)
					call print_var(bt(j) ,'ch4_tran bt(j)',idate,j,.false.,s)
					call print_var(ct(j) ,'ch4_tran ct(j)',idate,j,.false.,s)
					call print_var(rt(j) ,'ch4_tran rt(j)',idate,j,.false.,s)
					call print_var(epsilon_t_old(j,s) ,'ch4_tran epsilon_t_old(j,s)',idate,j,.false.,s)
					call print_var(source_old(j,s) ,'ch4_tran source_old(j,s)',idate,j,.false.,s)
		...
		do j = 1,nl_soil
			...
			err6 = err2 + err6
			err7 = err3 + err7
			call print_var(err1,'ch4_tran err1',idate,j,.true.)
			call print_var(err2,'ch4_tran err2',idate,j,.true.)
			call print_var(err3,'ch4_tran err3',idate,j,.true.)
			call print_var(err4,'ch4_tran err4',idate,j,.true.)

			call print_var(err5,'ch4_tran err5',idate,j,.true.)
			call print_var(err6,'ch4_tran err6',idate,j,.true.)
			call print_var(err7,'ch4_tran err7',idate,j,.true.)

			call print_var(conc_ch4(j),'ch4_tran conc_ch4(j)',idate,j,.true.)
			call print_var(conc_ch4_bef(j),'ch4_tran conc_ch4_bef(j)',idate,j,.true.)
			call print_var(ch4_prod_depth(j),'ch4_tran ch4_prod_depth(j)',idate,j,.true.)
			call print_var(ch4_oxid_depth(j),'ch4_tran ch4_oxid_depth(j)',idate,j,.true.)
			call print_var(errch4,'ch4_tran errch4',idate,j,.true.)
		end do
	...
	subroutine split_ch4_o2_phases( dz_soisno, wliq_soisno, porsl, &
		...
		!-----------------------------------------------------------
		do j = 1, nl_soil 
			call print_var(wliq_soisno,'wliq_soisno',idate)
			call print_var(dz_soisno,'dz_soisno',idate)
			call print_var(porsl,'porsl',idate)
