   ...
   SUBROUTINE hist_out (idate, deltim, itstamp, etstamp, ptstamp, &
      ...
      IF (lwrite) THEN
        ...
        CALL write_history_variable_2d ( DEF_hist_vars%vf, &
         ...
         ! heterotrophic respiration in soil layers
         CALL write_history_variable_3d ( DEF_hist_vars%hr_vr, &
            a_hr_vr, file_hist, 'f_hr_vr', itime_in_file, 'soil', 1, nl_soil, &
            sumarea, filter,'total vertically-resolved het. resp. from decomposing C pools','gC/m3/s')

         ! fraction of potential heterotrophic respiration
         CALL write_history_variable_3d ( DEF_hist_vars%fphr, &
            a_fphr, file_hist, 'f_fphr', itime_in_file, 'soil', 1, nl_soil, &
            sumarea, filter,'fraction of potential heterotrophic respiration','unitless')

         ! potential soil nitrification flux 
         CALL write_history_variable_3d ( DEF_hist_vars%pot_f_nit_vr, &
            a_pot_f_nit_vr, file_hist, 'f_pot_f_nit_vr', itime_in_file, 'soil', 1, nl_soil, &
            sumarea, filter,'potential soil nitrification flux','gN/m3/s')
